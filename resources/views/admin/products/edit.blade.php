...
@section("content")
    <div class="inline-flex flex-col items-start justify-between gap-2.5 p-5 w-full">
        {{-- heading --}}
        <div class="mb-10">
        ...
        {{-- form --}}
        <form class="flex flex-wrap lg:flex-nowrap gap-2.5 w-full" id="product" action="{{ route("admin.products.update", $product->id) }}" method="POST">
            @csrf
            @method("PATCH")

            {{-- left form --}}
            <div
                class="inline-flex bg-white w-full min-w-96 flex-col items-start justify-start gap-2.5 rounded border border-zinc-200 p-2.5">
                <div class="text-base font-semibold leading-tight text-gray-400">Name</div>
                    <input class="self-stretch rounded border border-zinc-300  p-2.5 focus:outline-none" id="name"
                    ...
                    <textarea class="w-full text-black rounded border border-zinc-300 p-2.5 focus:outline-none" id="description"
                        name="description" rows='8'  placeholder="Write something awesome...">{{ $product->description }}</textarea>
        ...
        {{-- right form --}}
            <div
                class="inline-flex min-w-[600px] flex-col items-start gap-[17px] rounded border border-zinc-200 bg-white px-[15px] py-[19px]">
